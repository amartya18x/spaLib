    ...
    for sizeCtr = 1:numSizes
        ...
        for sCtr = 1:numSFactors
            ...
            for run = 1:numRuns
                ...
                tStart = tic;
                [temp, errNorm(algoCtr,sizeCtr,sCtr,run), errDiff(algoCtr,sizeCtr,sCtr,run) numItr(algoCtr,sizeCtr,sCtr,run)] = grades_lin(samples.X(1:sizes(sizeCtr),:), samples.y(1:sizes(sizeCtr),run), samples.theta, sFactors(sCtr)*sBase, eta , mxitr, tol, vtol);
